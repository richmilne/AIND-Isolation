...
def play_match(player1, player2):
    ...
    return (num_wins[player1], num_timeouts[player1],
...
def main():
    ...
    def create_agent(args, heuristic):
        prefix = 'MM' if args['method'] == 'minimax' else 'AB'
        depth = 'inf' if args['iterative'] else str(args['search_depth'])
        name = '%s_%s_%s' % (prefix, heuristic[0], depth)
        player = CustomPlayer(score_fn=heuristic[1], **args)
        return Agent(player, name)
    ...
    mm_agents = [create_agent(MM_ARGS, h) for h in HEURISTICS]
    ab_agents = [create_agent(AB_ARGS, h) for h in HEURISTICS]
